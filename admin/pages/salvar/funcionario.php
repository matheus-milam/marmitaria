<?php
    ...
    if ($_POST) {
        ...
        $id_cargo = trim($_POST["id_cargo"] ?? NULL);
        $nr_telefone = trim($_POST["nr_telefone"] ?? NULL);
        $nr_salario = trim($_POST["nr_salario"] ?? NULL);

        if (empty($nm_funcionario) || empty($id_cargo) || empty($nr_salario)) {
            echo "<script>alert('Preencha nome, cargo e salário');history.back();</script>";
        } else {

            if (empty($id_funcionario)) {
                $sql = "insert into funcionario (id_funcionario, nm_funcionario, id_cargo, nr_telefone, nr_salario)
                values (NULL, :nm_funcionario, :id_cargo, :nr_telefone, :nr_salario)";
                ...
                $sql = "update funcionario set nm_funcionario = :nm_funcionario, id_cargo = :id_cargo, nr_telefone = :nr_telefone, nr_salario = :nr_salario where id_funcionario = :id_funcionario limit 1";
                $consulta = $pdo->prepare($sql);
                $consulta->bindParam(":nm_funcionario", $nm_funcionario);
                $consulta->bindParam(":id_cargo", $id_cargo);
                $consulta->bindParam(":nr_telefone", $nr_telefone);
